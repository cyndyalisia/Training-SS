...
?>
    ...
    <h2>Admin Page</h2>
      ...
      <table class="table" border="1">
        ...
        <?php
          global $wpdb;
          $result = $wpdb -> get_results('SELECT * FROM wordpress');
          if(isset($_GET['del'])){
            $del = $_GET['id'];
            $wpdb->query($wpdb->prepare("DELETE FROM wordpress WHERE ID = %d",$del));
          }
...
class Testimonial_Widget extends WP_Widget {
    ...
    global $wpdb;
    $mylink =$wpdb->get_row("SELECT testimonial FROM wordpress ORDER BY RAND() LIMIT 1");
